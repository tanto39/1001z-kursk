...
$(document).ready(function() {
     ...
     */
    var arImages = [];
    $('.detail-image-small-item').each(function(){
        arImages[arImages.length] = {
            href: $(this).data('full-src')
        }
    });

    var srcBig = arImages[0]['href'];

    $(".product .detail-image-big-wrap").html(
        "<figure class='detail-image-big flex'>" +
            "<img itemprop='image' class='detail-image-big-img' alt='Товар' src='" + srcBig + "'>" +
        "</figure>"
    );

    $(".detail-image-big").on("click", function () {
        blueimp.Gallery(
            arImages,
            {
                container: '#blueimp-gallery-carousel',
                index: $('.detail-image-small-item.active').index(),
                carousel: true,
                transitionSpeed: 800
            }
        );
    ...
    $(".product-list .detail-image-small-item").on("click", function () {
        blueimp.Gallery(
            arImages,
            {
                container: '#blueimp-gallery-carousel',
